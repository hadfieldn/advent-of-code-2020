defmodule Day11 do
  @moduledoc """
  Day 11: Find the number of occupied seats when the seating pattern stabilizes.
  """
  ...
  @doc """
    iex> Day11.get_layout(~w(L.L LL# L.#)) |> Day11.occupancy()
    2
  """
  ...
  @doc """
  iex> grid = Day11.to_indexed_grid(~w(... LL. LLL))
  ...> Day11.next_state({1, 1}, grid)
  "#"

  iex> grid = Day11.to_indexed_grid(~w(.#. ##. ###))
  ...> Day11.next_state({1, 1}, grid)
  "L"
  ...
  def next_state(pos, grid) do
    case {grid[pos], adjacent_occupancy(pos, grid)} do
      ...
      {"#", occupancy} when occupancy >= 4 -> "L"
      {state, _} -> state
    end
  end

  @doc """
  iex> grid = Day11.to_indexed_grid(~w(... L## ##L))
  ...> Day11.adjacent_occupancy({1, 1}, grid)
  3
  ...
  8
  """
  def adjacent_occupancy({row, col}, grid) do
  ...
  @doc """
  iex> Day11.get_layout(~w(L.L LLL L..))
  %{width: 3, height: 3, grid: ~w(L.L LLL L..) |> Day11.to_indexed_grid()}
  """
  ...
  iex> Day11.to_indexed_grid(~w(... .LL ...))
